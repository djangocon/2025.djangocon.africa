...
def test_parse_budget_valid_input():
    budget_text = "Travel: $500\nAccommodation: $300\nTotal: $800"
    result = parse_budget(budget_text)
    expected = {
        "total_amount": "800$",
        "items": [
            {"name": "Travel", "cost": "$500"},
            {"name": "Accommodation", "cost": "$300"}
        ]
    }
    assert result == expected

def test_parse_budget_empty_input():
    budget_text = ""
    result = parse_budget(budget_text)
    expected = {"total_amount": "0$", "items": []}
    assert result == expected

def test_parse_budget_no_total():
    budget_text = "Travel: $500\nAccommodation: $300"
    result = parse_budget(budget_text)
    expected = {
        "total_amount": "0$",
...
def test_parse_budget_complex_item_names():
    budget_text = "Air Travel Cost: $600\nHotel Stay: $400\nTotal: $1000"
    result = parse_budget(budget_text)
    expected = {
        "total_amount": "1000$",
        "items": [
            {"name": "Air Travel Cost", "cost": "$600"},
            {"name": "Hotel Stay", "cost": "$400"}
        ]
    }
    assert result == expected
