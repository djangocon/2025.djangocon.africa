...
def send_email(subject, text_content, html_content, from_email, from_name, to_email, to_name=None):
    """
    ...
    if settings.EMAIL_BACKEND == 'django.core.mail.backends.console.EmailBackend':
        ...
        try:
            ...
            logger.info(f"Mailjet response: Status={result.status_code}, Data={result.json()}")
            if result.status_code == 200:
                return True
            else:
                logger.error(f"Mailjet failed: Status={result.status_code}, Data={result.json()}")
                return False
        except Exception as e:
